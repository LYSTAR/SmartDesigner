    ...
    <el-row class="layout-search">
      <el-col :span="5">
        <FileInput @change="(files: File[]) => drawMaterial(files)">
          <el-tooltip placement="top" :hide-after="0" :content="$t('message.searchMat')">
      ...
      <el-col :span="19">
        <el-input :prefix-icon="Search" :placeholder="$t('message.searchMat')"></el-input>
      </el-col>
    </el-row>
    <div>
      <el-tabs v-model="activeMaterial" class="layout-tabs material-tab">
        ...
        <el-tab-pane :label="$t('message.myBookmarksMat')" name="self">{{ $t('message.myBookmarksMat') }}</el-tab-pane>
        <el-tab-pane :label="$t('message.myPurchasesMat')" name="team">{{ $t('message.myPurchasesMat') }}</el-tab-pane>
      </el-tabs>
    </div>
  </div>
</template>
