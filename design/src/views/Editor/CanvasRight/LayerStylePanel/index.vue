<template>
  <div @click.stop="cancelElement">
    <el-row class="layout-search">
      <el-input v-model="keywords" :prefix-icon="Search" :placeholder="$t('message.searchLayer')"></el-input>
    </el-row>
...
const layerObjects = computed(() => {
  ...
  if (!_keywords)
    return currentTemplate.value.objects
      .filter(item => !WorkSpaceCommonType.includes(item.id) && item.type.toLowerCase() !== ElementNames.REFERENCELINE)
      .reverse()
  return currentTemplate.value.objects
    .filter(
      item =>
        !WorkSpaceCommonType.includes(item.id) &&
