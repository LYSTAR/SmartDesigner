<template>
  <div class="image-style-panel">
    ...
    <div
      class="origin-image"
      :style="{
        backgroundImage: `url(${handleElement.originSrc ? handleElement.originSrc : handleElement.getSrc()})`,
      }"
    ></div>
    ...
    <el-row class="mt-10">
      ...
      <el-col :span="12">
        <el-button class="matting-button" @click="openMatting"> <IconMagicWand class="btn-icon" /> 抠图 </el-button>
      </el-col>
    </el-row>
    ...
    <el-row>
      <FileInput class="full-width-btn" @change="(files: FileList) => replaceImage(files)">
        <el-button class="full-btn"> <IconTransform class="btn-icon" />{{ $t('style.replaceimage') }} </el-button>
      </FileInput>
    </el-row>
    <el-row>
