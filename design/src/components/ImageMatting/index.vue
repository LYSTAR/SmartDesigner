  ...
  >
    ...
    >
      ...
      <div class="el-upload__text">
        {{ t('message.dragAndDropFiles') }}<em>{{ t('message.selectFileUpload') }}</em>
      </div>
      <template #tip>
    ...
    <template #footer>
      <div class="dialog-footer">
        <el-button v-show="state.originImage && state.toolModel" type="danger" @click="clear">{{
          t('message.clear')
        }}</el-button>
        <el-button v-show="state.resultImage" type="primary" @click="edit">{{ t('message.edit') }}</el-button>
        <el-button v-show="state.resultImage && state.toolModel" type="success" @click="download">{{
          t('message.download')
        }}</el-button>
        <el-button v-show="state.resultImage && !state.toolModel" v-loading="state.loading" type="primary">
          {{ state.loading ? t('message.uploading') : t('message.completeImageCutout') }}
        </el-button>
        <el-button
...
const download = () => {
  ...
  downloadLinkFile(state.resultImage, `SmartDesigner-${Date.now()}-matting-${state.filename}`)
}
