<template>
  ...
  >
    ...
    >
      ...
      <div class="el-upload__text">拖拽图片到这里 或者 <em>选择图片上传</em></div>
      <template #tip>
    ...
    <template #footer>
      <div class="dialog-footer">
        <el-button v-show="!showUpload && dataURL" type="danger" @click="clear">{{ t('message.clear') }}</el-button>
        <el-button v-show="!showUpload && dataURL" type="success" @click="download">{{
          t('message.download')
        }}</el-button>
      </div>
    </template>
...
const download = () => {
  ...
  downloadLinkFile(canvas.toDataURL('image/png'), `SmartDesigner-${Date.now()}-ImageFillColor.png`)
}
</script>
