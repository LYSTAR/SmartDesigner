...
export function createRotateMatrix({ angle = 0 }: TRotateMatrixArgs = {}, { x = 0, y = 0 }: Partial<XY> = {}): TMat2D {
...
export const createScaleMatrix = (x: number, y: number = x): TMat2D => [x, 0, 0, y, 0, 0]

export const angleToSkew = (angle: TDegree) => Math.tan(degreesToRadians(angle))

export const createSkewXMatrix = (skewValue: TDegree): TMat2D => [1, 0, angleToSkew(skewValue), 1, 0, 0]

export const createSkewYMatrix = (skewValue: TDegree): TMat2D => [1, angleToSkew(skewValue), 0, 1, 0, 0]

export const createTranslateMatrix = (x: number, y = 0): TMat2D => [1, 0, 0, 1, x, y]

export const multiplyTransformMatrixArray = (matrices: (TMat2D | undefined | null | false)[], is2x2?: boolean) =>
  matrices.reduceRight(
    (product: TMat2D, curr) => (curr ? multiplyTransformMatrices(curr, product, is2x2) : product),
    iMatrix
  )
