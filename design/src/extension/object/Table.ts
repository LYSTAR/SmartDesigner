...
export class Table extends Group {
  ...
  public onSet(options: { [key: string]: any }) {
    const dirty = this._columnsmodified || this._rowsmodified || this._cellsmodified
    if (!dirty) {
      return
    }
  ...
  getSelectionBounds(): TableSelectionBounds | null {
    ...
    const xmin = this.selection.reduce((min, p) => (p.c.index < min ? p.c.index : min), c.c.index)
    const xmax = this.selection.reduce(
      (max, p) => (p.c.index + (p.colspan || 1) - 1 > max ? p.c.index + (p.colspan || 1) - 1 : max),
      c.c.index + (c.colspan || 1) - 1
    )
    const ymin = this.selection.reduce((min, p) => (p.r.index < min ? p.r.index : min), c.r.index)
    const ymax = this.selection.reduce(
      (max, p) => (p.r.index + (p.rowspan || 1) - 1 > max ? p.r.index + (p.rowspan || 1) - 1 : max),
  ...
  insertRow(position = -1, height = this._rows[this._rows.length - 1].height || 1): void {
    if (position === -1) {
      const bounds = this.getSelectionBounds()
      if (bounds) {
    ...
    for (let x = 0; x < this._cols.length; x++) {
      ...
      const bottom = position < this._rows.length - 1 && this._cells[position + 1][x]
  ...
  getCells(options: TableCellDataOptions = {}): TableCellOutput[][] {
  ...
  private __setheight(newHeight: number) {
    if (this._rows.length) {
      const minHeigth = this._rows.slice(0, this._rows.length - 1).reduce((p: number, c) => p + c.height, 0)
  ...
  private __setwidth(newWidth: number) {
    if (this._cols.length) {
      const minWidth = this._cols.slice(0, this._cols.length - 1).reduce((p: number, c) => p + c.width, 0)
  ...
  private rowResizing(eventData: MouseEvent, transform: fabric.Transform, x: number, y: number, options: any = {}) {
    ...
    const zoom = this.canvas.getZoom()
    const newPoint = fabric.util.getLocalPoint(transform, transform.originX, transform.originY, x, y)
  ...
  private _deleteColumn(position = this._cols.length - 1): void {
    ...
    for (let y = 0; y < this._rows.length; y++) {
      ...
      const right = position < this._cols.length - 1 && this._cells[y][position + 1]
