...
export class ArcText extends OriginIText {
  ...
  override initDimensions() {
    ...
    for (let i = 0; i < this.__charBounds.length; i++) {
      ...
      const charOffset =
        heightOfLine - heightOfLine / this.lineHeight + (heightOfLine * this._fontSizeFraction) / this.lineHeight
      if (curvature > 0) {
  ...
  interateTextChunks(lineIndex: number, foo: Function, iteratorFn?: Function) {
    ...
    let shortCut =
      !isJustify && this.charSpacing === 0 && (!specs || !specs[lineIndex]) && this.isEmptyStyles(lineIndex)
