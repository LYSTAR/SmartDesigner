...
import { ObjectRef, Textbox, Object as FabricObject, Rect, Image as FabricImage } from 'fabric'
...
export default () => {
  ...
  const handleActive = <K extends keyof ObjectRef, T = ObjectRef[K] | undefined>(
    key: K
  ): WritableComputedRef<{
    modelValue: T
    ...
    const changeValue = (newValue: T, type: 'swipe' | 'change') => {
      ...
      if (['width', 'height', 'left', 'top', 'angle'].includes(key)) {
        ...
        // 图片元素通过 scaleX/scaleY 缩放，直接设置 width/height 会导致裁剪
        if ((key === 'width' || key === 'height') && activeObject instanceof FabricImage) {
          const naturalWidth = activeObject.getElement().naturalWidth || activeObject.width
          const naturalHeight = activeObject.getElement().naturalHeight || activeObject.height
          if (key === 'width') {
            activeObject.set('scaleX', Number(newValue) / naturalWidth)
          } else {
            activeObject.set('scaleY', Number(newValue) / naturalHeight)
          }
        } else {
        ...
        return {
          ...
          onChange: (value: T) => {
            ...
            canvas.fire('object:modified', {
              target: canvas.activeObject.value,
            })
          },
        }
      },
