...
export default () => {
  ...
  const exportImage = (format: ImageFormat, quality: number, dpi: number, ignoreClip = true) => {
    ...
    canvas
      .getObjects()
      .filter(item => item.type === ElementNames.REFERENCELINE && item.visible === true)
    ...
    saveAs(result, `SmartDesigner-${Date.now()}.${format}`)
    ...
    canvas
      .getObjects()
      .filter(item => item.type === ElementNames.REFERENCELINE && item.visible === false)
  ...
  const exportSVG = () => {
    ...
    downloadSVGFile(data, `SmartDesigner-${Date.now()}.svg`)
    ignoreObjects.map(item => item.set({ visible: true }))
    canvas
      .getObjects()
      .filter(obj => obj.id === WorkSpaceClipType)
  ...
  const convertFile = async (filetype: string, rangeType?: string) => {
    ...
    if (result && result.data.link) {
      downloadLinkFile(result.data.link, `SmartDesigner-${Date.now()}.${filetype}`)
    }
    Exporting.value = false
  }
  ...
  const exportJSON = () => {
    ...
    saveAs(blob, `SmartDesigner-${Date.now()}.json`)
  }
